...
class Memey(commands.Cog):
    ...
    @commands.command(
        ...
        aliases=[
            ...
            "giogiomeme",
            "itwasmedio",
            "youthoughtthiswasanormalcommandnamebutitwasmedio",
        ],
    )
    async def shitpostcrusaders(self, ctx):
        message_author = ctx.author
        print("{} issued .shitpostcrusaders 🌟".format(message_author))

        submissions = []

        try:
            for submission in reddit.subreddit("ShitPostCrusaders").top(
                "week", limit=125
            ):
                if submission and not submission.stickied and not submission.over_18:
                    submissions.append(submission)
